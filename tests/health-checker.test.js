...
describe('health-checker', () => {
  describe('parseTargets', () => {
    ...
    test('HEALTH_TARGETS를 올바르게 파싱한다', () => {
      process.env.HEALTH_TARGETS = 'nino:http://100.86.89.10:58090,rund:http://100.86.89.20:58090';
      const targets = parseTargets();
      expect(targets).toHaveLength(2);
      expect(targets[0]).toEqual({ name: 'nino', url: 'http://100.86.89.10:58090' });
  ...
  describe('analyzeHealth', () => {
    test('정상 상태면 이슈 없음', () => {
      ...
      expect(analyzeHealth('nino', data)).toEqual([]);
    });

    test('data가 null이면 연결 실패', () => {
      const issues = analyzeHealth('nino', null);
    ...
    test('tmux_alive=false 감지', () => {
      ...
      const issues = analyzeHealth('nino', data);
